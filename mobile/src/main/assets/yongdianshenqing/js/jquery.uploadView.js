...
(function($){  
	$.fn.uploadView = function(options){
		...
		var showBoximg = config.showBoximg;
		...
		var handleFileSelect = function(obj, _w, _h, _type, _size, _callback){
			  ...
			  reader.onload = (function(theFile){
				      return function (e) {
						...
						//alert(thisClosest.find(showBoximg).show())
						thisClosest.find(showBoximg).show().attr('src',tmpSrc); 
						//thisClosest.find(showBox).show().html(img);
						if (_w && _h) {
						  cssObj = { 'width':_w+'rem', 'height':_h+'rem' };
						} else if (_w) {
						  cssObj = { 'width':_w+'rem' };
						} else if (_h) {
						  cssObj = { 'height':_h+'rem' };
						} else {
						  cssObj = { 'max-width':'7rem', 'max-height':'4rem' };
						}
